...
- **API externe** : [ExchangeRate](https://www.exchangerate-api.com) — conversion des prix des véhicules (FCFA) vers d'autres devises (EUR, USD, etc.)
...
### Conversion de devises (API externe)
| Méthode | Route | Description |
|---|---|---|
| GET | `/exchange-rate/taux` | Liste de tous les taux de change depuis le FCFA |
| GET | `/exchange-rate/convertir/:montant?devise=EUR` | Convertit un montant FCFA vers une devise (ex: `/exchange-rate/convertir/22000000?devise=EUR`) |
...
Le dossier `docs-examen/git-github-collaboration/` contient toutes les commandes
Git Flow prêtes à l'emploi (création des branches `main`/`develop`/`feature/*`,
convention de commits clairs au format Conventional Commits, merges `--no-ff`,
tags de release) pour répondre à la section du sujet correspondante.
...
- **Architecture modulaire** : un module NestJS par ressource métier (`users`, `vehicules`, `reservations`, `notifications`, `exchange-rate`, `auth`), avec séparation entité / DTO / service / contrôleur.
...
- **API externe** : `ExchangeRateService` encapsule l'appel HTTP vers ExchangeRate-API via `HttpModule` (axios), avec gestion d'erreur dédiée (`BadGatewayException` si l'API externe est indisponible, `NotFoundException` si la devise est inconnue). Utilisée pour convertir les prix des véhicules (FCFA) vers d'autres devises.
